...
function DBService.GetSharedInventory(inventoryId, cb)
    MySQL.query("SELECT ci.character_id, ic.id, i.item, ci.amount, ic.metadata, ci.created_at, ci.degradation, ci.percentage, ci.slot FROM items_crafted ic\
...
function DBService.GetInventory(charIdentifier, inventoryId, cb)
    MySQL.query("SELECT ic.id, i.item, ci.amount, ic.metadata, ci.created_at, ci.character_id, ci.degradation, ci.slot FROM items_crafted ic\
		LEFT JOIN character_inventories ci on ic.id = ci.item_crafted_id\
...
function DBService.UpdateItemSlot(charIdentifier, itemCraftedId, slot)
...
function DBService.UpdateWeaponSlot(weaponId, slot)
    MySQL.update.await("UPDATE loadout SET slot = @slot WHERE id = @id;", {
        slot = slot,
        id = tonumber(weaponId)
    })
end
